...
int main()
{
    ...
    sprintf(TimeLengthFileMinimum,"ResultMinimum_manual/TimeLength.bin");
    sprintf(TimeLengthFileSaddle,"ResultSaddle_manual/TimeLength.bin"); 
    sprintf(X10File,"ResultSaddle_manual/X10.bin");
    sprintf(X20File,"ResultSaddle_manual/X20.bin");
    sprintf(Saddle_DisColumns_file_name,"ResultSaddle_manual/DisColumns.bin");
    sprintf(Saddle_DisRows_file_name,"ResultSaddle_manual/DisRows.bin");
    sprintf(Minimum_DisColumns_file_name,"ResultMinimum_manual/DisColumns.bin");
    sprintf(Minimum_DisRows_file_name,"ResultMinimum_manual/DisRows.bin");
    ...
    //下面是目视检查有问题的事例数以及事例的索引。
    // int ProblemNum = 10;
    // int ImageIndexSet[10] = {350, 360};
    int IndexSumimage = 18;
    for(int ImageIndex = 9; ImageIndex < 10; ImageIndex ++)
    {
        ...
        for(int subImageIndex = 0; subImageIndex < lensimagenum[ImageIndex]; subImageIndex ++ )
        {
            ...
            if(lenssnr[IndexSumimage] >= 12)
            {
                ...
                if((mur > 0)&&(mut > 0))
                {
                    sprintf(AreaName,"ResultMinimum_manual/Total%d_%dReadAreaMinimum.bin", IndexInAccept, SaveIndex);
                    sprintf(TimeName,"ResultMinimum_manual/Total%d_%dReadTimeMinimum.bin", IndexInAccept, SaveIndex); 
                    ofstream fp_area;
                    ofstream fp_time;
                    ...
                    //     DisRowsMax = 0;
                    // }
                    DisColumnsMax = DisColumnsMax / 2;
                    DisRowsMax = DisRowsMax / 2;
                ...
                if(mur * mut < 0)
                {
                    ...
                    sprintf(AreaName,"ResultSaddle_manual/Total%d_%dReadAreaSaddle.bin", IndexInAccept, SaveIndex);
                    sprintf(TimeName,"ResultSaddle_manual/Total%d_%dReadTimeSaddle.bin", IndexInAccept, SaveIndex);  
                    ofstream fp_area;
                    ofstream fp_time;
                    fp_area.open(AreaName, std::ofstream::binary);
                    fp_time.open(TimeName, std::ofstream::binary);
