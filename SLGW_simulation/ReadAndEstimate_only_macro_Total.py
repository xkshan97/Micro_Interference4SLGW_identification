    ...
    for j_ in range(imag_num):
        ...
        if SNR_network[IndexSumimage] > 12:
            if SNR_network[IndexSumimage] in SNR_selected:
                ...
                outdir = 'outdir_Lensed_' + Lens_Type + '/outdir' + str(i) + "_" + str(save_index)
                ...
                fname_list = ['./Sim_GW_Data_Lensed_' + Lens_Type + '/H1/H-H1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[Index_inject]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf' , './Sim_GW_Data_Lensed_' + Lens_Type + '/L1/L-L1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[Index_inject]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf', './Sim_GW_Data_Lensed_' + Lens_Type + '/V1/V-V1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[Index_inject]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf']
                ...
                for det_index , det in enumerate(["H1", "L1", "V1"]):#(["H1", "L1", "V1", "K1"]):
                    ...
                    print('SampleParam9_td = ', SampleParam9_td)
                    print('Inject geocent merger time = ', injection_parameters['geocent_time'])
                ...
                if H1_snr[IndexSumimage] >= L1_snr[IndexSumimage] and H1_snr[IndexSumimage] >= V1_snr[IndexSumimage]:
                    priors["H1_time"] = bilby.core.prior.Uniform(
                        minimum=trigger_time_H1[IndexSumimage] - 0.1,
                        maximum=trigger_time_H1[IndexSumimage] + 0.1,
                        name="H1_time",
                        latex_label="$t_H$",
                        unit="$s$",
                    )
                    ...
                    priors["L1_time"] = bilby.core.prior.Uniform(
                        minimum=trigger_time_L1[IndexSumimage] - 0.1,
                        maximum=trigger_time_L1[IndexSumimage] + 0.1,
                        name="L1_time",
                        latex_label="$t_L$",
                        unit="$s$",
                    )
                    ...
                    priors["V1_time"] = bilby.core.prior.Uniform(
                        minimum=trigger_time_V1[IndexSumimage] - 0.1,
                        maximum=trigger_time_V1[IndexSumimage] + 0.1,
                        name="V1_time",
                        latex_label="$t_V$",
                        unit="$s$",
                    )
