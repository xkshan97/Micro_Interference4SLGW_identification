...
def Generate_file(IndexStart, IndexEnd):
    ...
    for i in tqdm(range(IndexStart, IndexEnd)):
        ...
        plt.xlabel('t[s]')
        plt.ylabel('strain')
        plt.savefig('./IntermediaPlot_Gen_inter_precessing/waveform_' + str(i) + '.png', dpi=450)
        plt.close()
        ...
        frame.write_frame('./Sim_GW_Data_precessing/H1/H-H1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_h1.start_time.gpsSeconds) + '-4096_' + str(i) + '.gwf','H1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_h1)
        frame.write_frame('./Sim_GW_Data_precessing/L1/L-L1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_l1.start_time.gpsSeconds) + '-4096_' + str(i) + '.gwf','L1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_l1)
        frame.write_frame('./Sim_GW_Data_precessing/V1/V-V1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_v1.start_time.gpsSeconds) + '-4096_' + str(i) + '.gwf','V1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_v1)
        
            
        del hoft_h1, hoft_l1, hoft_v1, psd_h1_estimated, psd_l1_estimated, psd_v1_estimated, hp_orign, hc_orign, hp_20, hc_20, hp_h1, hc_h1, hp_l1, hc_l1, hp_v1, hc_v1, signal_h1, signal_l1, signal_v1, tilde_signal_h1, tilde_signal_l1, tilde_signal_v1, freq_h1, freq_l1, freq_v1, signal_with_noise_h1, signal_with_noise_l1, signal_with_noise_v1
        gc.collect() 
    return H1_snr_set, L1_snr_set, V1_snr_set, trigger_time_H1, trigger_time_L1, trigger_time_V1, gw_duration, gw_duration_20, Error, dt_h1_from_earth_center, dt_l1_from_earth_center, dt_v1_from_earth_center, spin1x, spin2x, spin1y, spin2y, spin1z, spin2z
...
if __name__=='__main__':
    ...
    length = int(267) #len(SampleParam[0])
    ...
    Result1, Result2, Result3, Result4, Result5, Result6, Result7, Result8, Result9, Result10, Result11, Result12, Result13, Result14, Result15, Result16, Result17, Result18  = list(res_z[0].get()[0]), list(res_z[0].get()[1]), list(res_z[0].get()[2]), list(res_z[0].get()[3]), list(res_z[0].get()[4]), list(res_z[0].get()[5]), list(res_z[0].get()[6]), list(res_z[0].get()[7]), list(res_z[0].get()[8]), list(res_z[0].get()[9]), list(res_z[0].get()[10]), list(res_z[0].get()[11]), list(res_z[0].get()[12]), list(res_z[0].get()[13]), list(res_z[0].get()[14]), list(res_z[0].get()[15]), list(res_z[0].get()[16]), list(res_z[0].get()[17])
    for i in range(threadcount-1):
        ...
        Result16.extend(list(res_z[i+1].get()[15]))
        Result17.extend(list(res_z[i+1].get()[16]))
        Result18.extend(list(res_z[i+1].get()[17]))
        
       
    
np.savetxt('./Sim_GW_Data_precessing/H1/H1_snr.csv', Result1, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/L1/L1_snr.csv', Result2, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/V1/V1_snr.csv', Result3, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/trigger_time_H1.csv', Result4, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/trigger_time_L1.csv', Result5, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/trigger_time_V1.csv', Result6, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/gw_duration.csv', Result7, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/gw_duration_20.csv', Result8, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/Error_index.csv', Result9, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/dt_h1_from_earth_center.csv', Result10, delimiter=',')
np.savetxt('./Sim_GW_Data_precessing/dt_l1_from_earth_center.csv', Result11, delimiter=',')      
np.savetxt('./Sim_GW_Data_precessing/dt_v1_from_earth_center.csv', Result12, delimiter=',')      
