...
def GenWaveFile(start_index, end_index): 
            ...
            if SNR_network[IndexSumimage] > 12:
                if SNR_network[IndexSumimage] in SNR_selected:
                    ...
                    if Lens_Type == "Minimum": 
                        M_L = 0.38
                        z_L = lens_z_set[i_]
                        fileArea = "../Paper4_MicroLens_Modify/ResultMinimum_0_300/Total" + str(i) + "_" + str(save_index) + "ReadAreaMinimum.bin"
                        fileTime = "../Paper4_MicroLens_Modify/ResultMinimum_0_300/Total" + str(i) + "_" + str(save_index) + "ReadTimeMinimum.bin"
                        ...
                        M_L = 0.38
                        z_L = lens_z_set[i_]
                        fileArea = "../Paper4_MicroLens_Modify/ResultSaddle_0_300/Total" + str(i) + "_" + str(save_index) + "ReadAreaSaddle.bin"
                        fileTime = "../Paper4_MicroLens_Modify/ResultSaddle_0_300/Total" + str(i) + "_" + str(save_index) + "ReadTimeSaddle.bin"
                    ...
                    spin1z_tmp = np.random.uniform(-SampleParam[3][i], SampleParam[3][i])
                    spin2z_tmp = np.random.uniform(-SampleParam[4][i], SampleParam[4][i])
                    spin1x_tmp = np.random.uniform(-np.sqrt(SampleParam[3][i] - spin1z_tmp ** 2), np.sqrt(SampleParam[3][i] - spin1z_tmp ** 2))
                    spin2x_tmp = np.random.uniform(-np.sqrt(SampleParam[4][i] - spin2z_tmp ** 2), np.sqrt(SampleParam[4][i] - spin2z_tmp ** 2))
                    spin1y_tmp = np.random.choice([-1,1]) * np.sqrt(SampleParam[3][i] - spin1z_tmp ** 2 - spin1x_tmp ** 2)
                    spin2y_tmp = np.random.choice([-1,1]) * np.sqrt(SampleParam[4][i] - spin2z_tmp ** 2 - spin2x_tmp ** 2)
                    
                    spin1z.append(spin1z_tmp)
                    spin2z.append(spin2z_tmp)
                    spin1x.append(spin1x_tmp)
                    spin2x.append(spin2x_tmp)
                    spin1y.append(spin1y_tmp)
                    spin2y.append(spin2y_tmp)
                    if np.abs(spin1x_tmp**2 + spin1y_tmp**2 + spin1z_tmp**2 - SampleParam[3][i]) > 1e-3 or np.abs(spin2x_tmp**2 + spin2y_tmp**2 + spin2z_tmp**2 - SampleParam[4][i]) > 1e-3:
                        print("Spin wrong !!!!!!!!!!")
                    ...
                    np.savetxt("../Paper4_MicroLens_Modify/Ffabs" + Lens_Type + "_precessing/Total" + str(i) + "_" + str(save_index) + ".csv", Ff_inter(freq_h1), delimiter=',')
                    np.savetxt("../Paper4_MicroLens_Modify/ThetaF" + Lens_Type + "_precessing/Total" + str(i) + "_" + str(save_index) + ".csv", ThetaF_inter(freq_h1), delimiter=',')
                    ...
                    plt.xlim(1, 1000)
                    plt.savefig('./IntermediaPlot_Gen_inter_Micro_precessing/Tilde_hf' + str(i) + '_' + str(save_index) + '.png', dpi=450)
                    plt.close()
                    ...
                    plt.xlabel('t[s]')
                    plt.ylabel('strain')
                    plt.savefig('./IntermediaPlot_Gen_inter_Micro_precessing/waveform_' + str(i) + '_' +str(save_index) +'.png', dpi=450)
                    plt.close()
                    
                    
                    frame.write_frame('./Total_Sim_GW_Data_Lensed_' + Lens_Type + '_precessing/H1/H-H1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_h1.start_time.gpsSeconds) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf','H1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_h1)
                    frame.write_frame('./Total_Sim_GW_Data_Lensed_' + Lens_Type + '_precessing/L1/L-L1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_l1.start_time.gpsSeconds) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf','L1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_l1)
                    frame.write_frame('./Total_Sim_GW_Data_Lensed_' + Lens_Type + '_precessing/V1/V-V1_GWOSC_' + sample_RATE + '_R1-' + str(hoft_v1.start_time.gpsSeconds) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf','V1:GWOSC_' + sample_RATE + '_R1_STRAIN',signal_with_noise_v1)
    ...
    return inject_time, trigger_time_H1, trigger_time_L1, trigger_time_V1, gw_duration, gw_duration_20, dt_h1_from_earth_center, dt_l1_from_earth_center, dt_v1_from_earth_center, spin1x, spin2x, spin1y, spin2y, spin1z, spin2z
...
if __name__=='__main__':
    ...
    pool.close()
    pool.join() 
    Result1, Result2, Result3, Result4, Result5, Result6, Result7, Result8, Result9, Result10, Result11, Result12, Result13, Result14, Result15 = list(res_z[0].get()[0]), list(res_z[0].get()[1]), list(res_z[0].get()[2]), list(res_z[0].get()[3]), list(res_z[0].get()[4]), list(res_z[0].get()[5]), list(res_z[0].get()[6]), list(res_z[0].get()[7]), list(res_z[0].get()[8]), list(res_z[0].get()[9]), list(res_z[0].get()[10]), list(res_z[0].get()[11]), list(res_z[0].get()[12]), list(res_z[0].get()[13]), list(res_z[0].get()[14])
    for i in range(threadcount-1):
        ...
        Result15.extend(list(res_z[i+1].get()[14]))

np.savetxt('./Lensed_SampleResult_Micro_precessing/Total_inject_time.csv', Result1)
np.savetxt('./Lensed_SampleResult_Micro_precessing/trigger_time_H1.csv', Result2, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/trigger_time_L1.csv', Result3, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/trigger_time_V1.csv', Result4, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/gw_duration.csv', Result5, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/gw_duration_20.csv', Result6, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/dt_h1_from_earth_center.csv', Result7, delimiter=',')
np.savetxt('./Lensed_SampleResult_Micro_precessing/dt_l1_from_earth_center.csv', Result8, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/dt_v1_from_earth_center.csv', Result9, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin1x.csv', Result10, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin2x.csv', Result11, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin1y.csv', Result12, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin2y.csv', Result13, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin1z.csv', Result14, delimiter=',')      
np.savetxt('./Lensed_SampleResult_Micro_precessing/spin2z.csv', Result15, delimiter=',')