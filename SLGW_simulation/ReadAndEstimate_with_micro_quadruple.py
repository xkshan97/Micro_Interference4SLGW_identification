...
Total_inject_time = np.loadtxt('./Lensed_SampleResult_Micro_quadruple/Total_inject_time.csv', delimiter=',')

Interferometer_List = ["H1", "L1", "V1"]
Interferometer_name = 'H1_L1_V1'
logger = bilby.core.utils.logger

IndexInAccept = np.loadtxt('../Paper5_cWB_Modify/Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/IndexInAccept.csv', delimiter=',')
SNR_selected = np.loadtxt('../Paper5_cWB_Modify/Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/SNR_selected.csv', delimiter=',')
SNR_Lens_gtr_12_sum = np.loadtxt('../Paper5_cWB_Modify/Theory_match_total_quadruple/SNR_total.csv', delimiter=',')
try:
    len(IndexInAccept)
except TypeError:
    IndexInAccept = [int(IndexInAccept)]
# for i_ in range(len(AcceptLensIndex[30:40])):
for i_ in IndexInAccept[29:28:-1]:
    i_ = int(i_)
    ...
    for j_ in range(imag_num):
        ...
        if SNR_network[IndexSumimage] > 12:
            if SNR_network[IndexSumimage] in SNR_selected:
                ...
                outdir = 'outdir_Lensed_' + Lens_Type + '_quadruple/outdir' + str(i) + "_" + str(save_index)
                ...
                fname_list = ['./Total_Sim_GW_Data_Lensed_' + Lens_Type + '_quadruple/H1/H-H1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[index_in_total_time]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf' , './Total_Sim_GW_Data_Lensed_' + Lens_Type + '_quadruple/L1/L-L1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[index_in_total_time]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf', './Total_Sim_GW_Data_Lensed_' + Lens_Type + '_quadruple/V1/V-V1_GWOSC_4KHZ_R1-'+str(int(Total_inject_time[index_in_total_time]))+'-4096_' + str(i) + "_" + str(save_index) + '.gwf']
