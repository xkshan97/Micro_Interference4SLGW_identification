...
df = 1.0 / duration
psd_l1_h1 = pycbc.psd.aLIGODesignSensitivityT1800044(fsamples, df, f_lower)
psd_v1 = pycbc.psd.AdVDesignSensitivityP1200087(fsamples, df, f_lower)
...
for i in tqdm(SNR_select_index[0:100]):
    i = int(i)
    try:
        ...
        #得到不同探测器的时间
        start_time_signal = int(SampleParam[9][i]) - (hp_orign.duration - hp_20.duration) #保证30Hz的开始时间是在start time上
        ...
        tilde_signal_h1_inj_func = interp1d(freq_h1_inj, tilde_signal_h1_inj)
        tilde_signal_l1_inj_func = interp1d(freq_l1_inj, tilde_signal_l1_inj)
                 
        index_plot = np.where((freq_h1 > 25)&(freq_h1 < 1024))
        
        # plt.title('SNR = ' + str(SNR_tmp))
        # plt.loglog(freq_h1[index_plot], np.abs(tilde_signal_h1[index_plot]), label = 'Bilby')
        # plt.loglog(freq_h1_inj[index_plot], np.abs(tilde_signal_h1_inj[index_plot]), '--', label='inj')
        # plt.xlim(1, 500)
        # # plt.ylim(10**(-1), 100)
        # plt.legend()
        # plt.savefig('IntermediaPlot_waveform_macro/H1_Micro_Tilde_res_vs_inj_' + str(i) + '.png', dpi=450)
        # plt.close()

        # plt.title('SNR = ' + str(SNR_tmp))
        # plt.loglog(freq_l1[index_plot], np.abs(tilde_signal_l1[index_plot]), label = 'Bilby')
        # plt.loglog(freq_l1_inj[index_plot], np.abs(tilde_signal_l1_inj[index_plot]), '--', label='inj')
        # plt.xlim(1, 500)
        # # plt.ylim(10**(-1), 100)
        # plt.legend()
        # plt.savefig('IntermediaPlot_waveform_macro/L1_Micro_Tilde_res_vs_inj_' + str(i) + '.png', dpi=450)
        # plt.close()

        # plt.title('SNR = ' + str(SNR_tmp))
        # plt.plot(signal_h1.sample_times, signal_h1, label='Bilby')
        # plt.plot(signal_h1_inj.sample_times, signal_h1_inj, '--', label='inj')
        # plt.xlim(geocent_time - 1, geocent_time + 0.1)
        # plt.legend()
        # plt.savefig('IntermediaPlot_waveform_macro/H1_Time_res_vs_inj_' + str(i) + '.png', dpi=450)
        # plt.close()

        # plt.title('SNR = ' + str(SNR_tmp))
        # plt.plot(signal_l1.sample_times, signal_l1, label='Bilby')
        # plt.plot(signal_l1_inj.sample_times, signal_l1_inj, '--', label='inj')
        # plt.xlim(geocent_time - 1, geocent_time + 0.1)
        # plt.legend()
        # plt.savefig('IntermediaPlot_waveform_macro/L1_Time_res_vs_inj_' + str(i) + '.png', dpi=450)
        # plt.close()
        ...
        """下面开始计算residual"""
        """读取数据"""
        fname_list = ['./Sim_GW_Data/H1/H-H1_GWOSC_4KHZ_R1-'+str(int(start_time - signal_duration/2))+'-4096_' + str(i) + '.gwf', './Sim_GW_Data/L1/L-L1_GWOSC_4KHZ_R1-'+str(int(start_time - signal_duration/2))+'-4096_' + str(i) + '.gwf']
        ...
        # plt.xlim(signal_h1.sample_times[0] + signal_h1.duration * 7 / 10, signal_h1.sample_times[0] + signal_h1.duration * 8 / 10)
        # plt.savefig('IntermediaPlot_waveform_macro/H1_Read_Time_res_vs_data_' + str(i) + '.png', dpi=450)
        # plt.close()
        ...
        # plt.xlim(signal_l1.sample_times[0] + signal_l1.duration * 7 / 10, signal_l1.sample_times[0] + signal_l1.duration * 8 / 10)
        # plt.savefig('IntermediaPlot_waveform_macro/L1_Read_Time_res_vs_data_' + str(i) + '.png', dpi=450)
        # plt.close()
        ...
        # plt.savefig('IntermediaPlot_waveform_macro/H1_Residual_' + str(i) + '.png', dpi=450)
        # plt.close()
        ...
        # # plt.plot(data_segment.times, signal_rec_inter(data_segment.times), '--' ,label='interp1d')
        # # plt.xlim(signal_h1.sample_times[0] + signal_h1.duration * 6 / 10, signal_h1.sample_times[0] + signal_h1.duration * 10 / 10)
        # plt.savefig('IntermediaPlot_waveform_macro/L1_Residual_' + str(i) + '.png', dpi=450)
        # plt.close()
        ...
        # plt.semilogy(freq_used_in_freq_residual_cal, np.abs(tilde_signal_h1_inj_func(freq_used_in_freq_residual_cal)), label='inj')
        ...
        # plt.savefig('IntermediaPlot_waveform_macro/H1_freq_domain_abs_' + str(i) + '.png', dpi=450)
        # plt.close()

        # plt.semilogy(freq_used_in_freq_residual_cal, np.abs(data_segment_func_l1(freq_used_in_freq_residual_cal)), label='data')
        
        # plt.semilogy(freq_used_in_freq_residual_cal, np.abs(tilde_signal_l1_inj_func(freq_used_in_freq_residual_cal)), label='inj')

        # plt.semilogy(freq_used_in_freq_residual_cal, np.abs(Bilby_func_l1(freq_used_in_freq_residual_cal)), label='bilby')
        # plt.legend()
        # plt.ylim(10**(-25), 10**(-20))
        # plt.savefig('IntermediaPlot_waveform_macro/L1_freq_domain_abs_' + str(i) + '.png', dpi=450)
        # plt.close()
...
np.savetxt('IntermediaPlot_waveform_macro/Residual_res_freq.csv', Residual_res_freq, delimiter=',')
np.savetxt('IntermediaPlot_waveform_macro/Residual_res_noise_freq.csv', Residual_res_noise_freq, delimiter=',')
np.savetxt('IntermediaPlot_waveform_macro/Residual_res_cross_freq.csv', Residual_res_cross_freq, delimiter=',')
np.savetxt('IntermediaPlot_waveform_macro/Residual_res_noise_cross_freq.csv', Residual_res_noise_cross_freq, delimiter=',')
np.savetxt('IntermediaPlot_waveform_macro/Residual_SNR.csv', Residual_SNR, delimiter=',')
