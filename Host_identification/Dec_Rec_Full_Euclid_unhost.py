...
Euclid_file = 'Euclid_unhost_25p5_long_time'

Index_in_accepted_Selected_size_less_theta_E_and_seeing_and_mag_less_25p5_and_4_images = np.loadtxt('SampleResult_Galaxy/Index_in_accepted_Selected_size_less_theta_E_and_seeing_and_mag_less_25p5_and_4_images.csv', delimiter=',')
...
for index in Index_in_accepted_Selected_size_less_theta_E_and_seeing_and_mag_less_25p5_and_4_images:
    ...
    kwargs_VIS_band['num_exposures'] = 24
    ...
    # lens light
    kwargs_lens_light_mag_VIS = [{'magnitude': Lens_light_mag[index], 'R_sersic': Lens_R_e[index], 'n_sersic': 4, 'e1': e1, 'e2': e2, 'center_x': 0, 'center_y': 0}]
    # source light
    e1_s, e2_s = param_util.phi_q2_ellipticity(phi=source_position_angle[index] * np.pi / 180, q=source_axis_ratio[index])
    kwargs_source_mag_VIS = [{'magnitude': apparent_mag_set[index], 'R_sersic': source_Re_circ[index], 'n_sersic': source_Sersic_n[index], 'e1': e1_s, 'e2': e2_s, 'center_x': source_x, 'center_y': source_y}]
    ...
    plt.savefig('./' + Euclid_file +'/' + str(index) + '_Sim_fig.png', dpi=450)
    plt.close()
    ...
    plt.savefig('./' + Euclid_file + '/' + str(index) + 'rec_obs.png', dpi=450)
    plt.close()
    ...
    f.tight_layout()
    f.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0., hspace=0.05)
    plt.savefig('./' + Euclid_file + '/' + str(index) + 'decomposition.png', dpi=450)
    plt.close()
    print(kwargs_result)
    ...
    for i in range(len(chain_list)):
        ...
        plt.savefig('./' + Euclid_file + '/' + str(index) + 'chain.png', dpi=450)
        plt.close() 
    ...
    if not samples_mcmc == []:
            ...
            plt.savefig('./' + Euclid_file + '/' + str(index) + 'corner_lens_full.png', dpi=450)
            plt.close() 
            plot = corner.corner(samples_mcmc[:,8:], labels=param_mcmc[8:], show_titles=True, truths=param_truths[8:])
            plt.savefig('./' + Euclid_file + '/' + str(index) + 'corner_lens_sub.png', dpi=450)
            plt.close() 
    ...
    def Generate_mcmc_td(IndexStart, IndexEnd):
        ...
        for i in tqdm(range(IndexStart, IndexEnd)):
            ...
            x_image_result_tmp, y_image_result_tmp = lensEquationSolver.findBrightImage(x_source_result_tmp, y_source_result_tmp, kwargs_lens_result_tmp, numImages=4,
                                                            min_distance=0.1*kwargs_VIS_band['pixel_scale'], search_window=numpix * kwargs_VIS_band['pixel_scale'])
    ...
    if __name__=='__main__':
        ...
        length = len(samples_mcmc)#int(threadcount * 150) #
    ...
    try:
        ...
        plt.savefig('./' + Euclid_file + '/' + str(index) + 'corner_Res.png', dpi=450)
        plt.close()
        np.savetxt('./' + Euclid_file + '/' + str(index) + 'corner_Res_data.csv', mcmc_new_list, delimiter=',')
