...
def Main_func(index_tmp):
    ...
    # lens light
    kwargs_lens_light_mag_r = [{'magnitude': GW_Lens_Galaxy_Res[0], 'R_sersic': GW_Lens_Galaxy_Res[1] / angular_diameter_distance_to_lens * u.rad.to(u.arcsec), 'n_sersic': 4, 'e1': e1, 'e2': e2, 'center_x': 0, 'center_y': 0}]
    # source light
    e1_s, e2_s = param_util.phi_q2_ellipticity(phi=mag_Res[index_tmp][2] * np.pi / 180, q=mag_Res[index_tmp][1])
    kwargs_source_mag_r = [{'magnitude': mag_Res[index_tmp][0], 'R_sersic': mag_Res[index_tmp][3], 'n_sersic': mag_Res[index_tmp][4], 'e1': e1_s, 'e2': e2_s, 'center_x': source_x, 'center_y': source_y}]

    # turn magnitude kwargs into lenstronomy kwargs
    kwargs_lens_light_r, kwargs_source_r, kwargs_ps_r = sim_r.magnitude2amplitude(kwargs_lens_light_mag_r, kwargs_source_mag_r)
    ...
    img = np.zeros((image_r.shape[0], image_r.shape[1], 1), dtype=float)
    img[:,:,0] = plot_util.sqrt(image_r)

    plot_size = numpix * kwargs_r_band['pixel_scale']
    ...
    plt.savefig('./' + CSST_file +'/' + str(index) + '_' + str(index_tmp) + '_Sim_fig.png', dpi=450)
    plt.close()

    kwargs_data_r = {'background_rms': sim_r.kwargs_data['background_rms'], 'exposure_time': sim_r.kwargs_data['exposure_time'], 'ra_at_xy_0': sim_r.kwargs_data['ra_at_xy_0'], 'dec_at_xy_0': sim_r.kwargs_data['dec_at_xy_0'], 'transform_pix2angle': sim_r.kwargs_data['transform_pix2angle'], 'image_data': image_r}

    image_band = [kwargs_data_r, sim_r.kwargs_psf, kwargs_numerics]
    ...
    kwargs_lens_init.append({'theta_E': theta_E, 'e1': e1, 'e2': e2,
                            'center_x': 0., 'center_y': 0.})
    kwargs_lens_sigma.append({'theta_E': 0.2, 'e1': 0.05, 'e2': 0.05,
                            'center_x': 0.05, 'center_y': 0.05})
    kwargs_lower_lens.append({'theta_E': 0.01, 'e1': -0.5, 'e2': -0.5, 'center_x': -1, 'center_y': -1})
    kwargs_upper_lens.append({'theta_E': 2., 'e1': 0.5, 'e2': 0.5, 'center_x': 1, 'center_y': 1})
    ...
    fixed_source.append({})
    kwargs_source_init.append({'R_sersic': mag_Res[index_tmp][3], 'n_sersic': mag_Res[index_tmp][4], 'e1': e1_s, 'e2': e2_s, 'center_x': source_x, 'center_y': source_y})
    ...
    plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'rec_obs.png', dpi=450)
    plt.close()
    ...
    f.tight_layout()
    f.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0., hspace=0.05)
    plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'decomposition.png', dpi=450)
    plt.close()
    print(kwargs_result)
    ...
    for i in range(len(chain_list)):
        ...
        plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'chain.png', dpi=450)
        plt.close()
        
    sampler_type, samples_mcmc, param_mcmc, dist_mcmc  = chain_list[1]

    param_class = fitting_seq.param_class
    param_truths = param_class.kwargs2args(kwargs_lens=kwargs_lens, kwargs_source=kwargs_source_r, kwargs_lens_light=kwargs_lens_light_r)
    ...
    if not samples_mcmc == []:
        ...
        plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_lens_full.png', dpi=450)
        plt.close() 
        plot = corner.corner(samples_mcmc[:,8:], labels=param_mcmc[8:], show_titles=True, truths=param_truths[8:])
        plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_lens_sub.png', dpi=450)
        plt.close() 
    ...
    for i in tqdm(range(len(samples_mcmc))):
        ...
        x_image_result_tmp, y_image_result_tmp = lensEquationSolver.findBrightImage(x_source_result_tmp, y_source_result_tmp, kwargs_lens_result_tmp, numImages=4,
                                                        min_distance=0.5*kwargs_r_band['pixel_scale'], search_window=numpix * kwargs_r_band['pixel_scale'])
    ...
    for i in range(len(theta_E_result)):
        mcmc_new_list.append([theta_E_result[i], center_x_result[i], center_y_result[i], e1_result[i], e2_result[i], t_days_0_result[i], t_days_1_result[i], t_days_2_result[i], t_days_3_result[i]])
                
    x_image_truths, y_image_truths = lensEquationSolver.findBrightImage(source_x, source_y, kwargs_lens, numImages=4,
                                                        min_distance=0.1*kwargs_r_band['pixel_scale'], search_window=numpix * kwargs_r_band['pixel_scale'])
    ...
    try:
        ...
        plt.savefig('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_Res.png', dpi=450)
        plt.close()
        np.savetxt('./' + CSST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_Res_data.csv', mcmc_new_list, delimiter=',')
