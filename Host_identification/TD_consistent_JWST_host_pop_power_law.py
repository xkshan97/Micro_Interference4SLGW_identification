...
for index_tmp in range(1):#range(len(JWST_mag_Res)):

    samples_mcmc = np.loadtxt('./' + JWST_file + '/' + str(index) + '_' + str(index_tmp) + 'samples_mcmc.csv', delimiter=',')
    kwargs_result = np.load('./' + JWST_file + '/' + str(index) + '_' + str(index_tmp) + 'kwargs_result.npy', allow_pickle=True).item()
    ...
    kwargs_model_td = {'lens_model_list': ['EPL'],
    ...
    lens_model_class = LensModel(lens_model_list=['EPL'], z_lens=z_lens, z_source=z_source, cosmo=cosmo)

    lensEquationSolver = LensEquationSolver(lens_model_class)
    # make instance of parameter class with given model options, constraints and fixed parameters #
    kwargs_model = {'lens_model_list': ['EPL'],  # list of lens models to be used
    ...
    def TD_consistent(index_start, index_end):
        ...
        for index_in_mcmc in tqdm(range(index_start, index_end)):
            ...
            gamma_result_tmp = kwargs_result_tmp['kwargs_lens'][0]['gamma']
            ...
            kwargs_sie_result_tmp = {'theta_E': theta_E_result_tmp, 'gamma': gamma_result_tmp, 'center_x': center_x_result_tmp, 'center_y': center_y_result_tmp, 'e1': e1_result_tmp, 'e2': e2_result_tmp}  # parameters of the deflector lens model
            ...
            for tmp_i in range(1000):
                ...
                if len(x_image_result_tmp) == 4:
                    ...
                    gamma_result.append(gamma_result_tmp)
        ...
        for i in range(len(theta_E_result)):
            mcmc_new_list.append([theta_E_result[i], gamma_result[i], center_x_result[i], center_y_result[i], e1_result[i], e2_result[i], t_days_0_result[i], t_days_1_result[i], t_days_2_result[i], t_days_3_result[i], SFR_set[i]])
    ...
    kwargs_lens = [
            {'theta_E': theta_E, 'gamma': 2, 'e1': e1, 'e2': e2, 'center_x': 0, 'center_y': 0}  # SIE model
        ]
    ...
    labels_new = [r"$theta_E$", r'$\gamma$' ,r"$center_x$", r"$center_y$", r"$e_1$", r"$e_2$", r"$t_0$", r"$t_1$", r"$t_2$", r"t_3", r"SFR"]
    try:
        plot = corner.corner(np.array(Result1), labels=labels_new, show_titles=True, truths=[kwargs_lens[0]['theta_E'], kwargs_lens[0]['gamma'], kwargs_lens[0]['center_x'], kwargs_lens[0]['center_y'], kwargs_lens[0]['e1'], kwargs_lens[0]['e2'], t_days_truths[0], t_days_truths[1], t_days_truths[2], t_days_truths[3], 0])
        plt.savefig('./' + JWST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_Res.png', dpi=450)
        plt.close()
        np.savetxt('./' + JWST_file + '/' + str(index) + '_' + str(index_tmp) + 'corner_Res_data.csv', Result1, delimiter=',')
