...
def normalize(pdf,  args=(), lower_bd=-np.inf, upper_bd=np.inf, vectorize=False):
    ...
    if len(args)==0:
        ...
        quadrature = quad(pdf, lower_bd, upper_bd,full_output=1,args=args)
    ...
    if len(args)==0:
        ...
        pdf_normed = lambda x: pdf(x, args[0])/A if lower_bd <= x <= upper_bd else 0
...
def get_cdf(pdf, args=(), lower_bd=-np.inf, upper_bd=np.inf):
    ...
    """
    pdf_norm = normalize(pdf, args, lower_bd, upper_bd)
    ...
    def cdf_number(x):
        ...
        if x < lower_bd:
            ...
            return quad(pdf_norm,lower_bd,x,limit=100)[0]
...
def sample(pdf, num_samples, args=(), lower_bd=-np.inf, upper_bd=np.inf, guess=0, chebyshev=False):
    ...
    if chebyshev:
        ...
        cdf = get_cdf(pdf, args, lower_bd, upper_bd)
