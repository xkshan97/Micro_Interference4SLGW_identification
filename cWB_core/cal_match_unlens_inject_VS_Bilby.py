...
with open('./Data_start_time_and_snr_and_match_and_outdir_name_complex/max_match_det_unlens.csv', 'r') as f1:
...
for i in tqdm(range(len(inject_time_set))):
    ...
    # time_jitter = data_unlens['posterior']['content']['time_jitter'][j]
    geocent_time = data_unlens['posterior']['content']['geocent_time'][j]
    iota = data_unlens['posterior']['content']['iota'][j]
    ...
    match_tmp_h1_noise = Match((np.abs(Bilby_func_h1(freq_match)))/func_psd(freq_match)**(1/2), (np.abs(Bilby_func_h1_noise(freq_match)))/func_psd(freq_match)**(1/2))
    match_tmp_h1 = Match((np.abs(Bilby_func_h1(freq_match)))/func_psd(freq_match)**(1/2), (np.abs(Bilby_func_h1(freq_match)))/func_psd(freq_match)**(1/2))
    ...
    plt.legend()
    plt.grid()
    plt.savefig('IntermediaPlot/Unlens_selected/noisy_freq_' + file_name[i].split('/')[3][6::] + '.png', dpi=450)
    plt.close()

np.savetxt('./match_noisy_inject_VS_Bilby/unlens_match_noise.csv', match_noise, delimiter=',')
np.savetxt('./match_noisy_inject_VS_Bilby/unlens_match_no_noise.csv', match_no_noise, delimiter=',')
