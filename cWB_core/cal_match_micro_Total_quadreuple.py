...
with open('./Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/reference_det.csv', 'r') as f1:
...
#读取out_dir中的数据
# freq_max = np.loadtxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/ringdown_freq.csv', delimiter=',', encoding='UTF-8-sig') + 100
...
for i in tqdm(range(len(inject_time_set))):
    ...
    plt.semilogy(freq_match, func_rec_max(freq_match))
    plt.semilogy(freq_match, abs(Bilby_func_max(freq_match)), '--')
    plt.plot([freq_max[i], freq_max[i]], [10**(-30), 10**(-22)])
    plt.xlim(10, 500)
    plt.ylim(10**(-30),10**(-22))
    plt.grid()
    plt.savefig("./IntermediaPlot_match_lensed_quadruple/" + file_name[i].split('/')[3][6::] + "_" + str(inject_time_set[i]) + ".png", dpi=450)
    plt.close()
    ...
    np.savetxt('./Bilby_rec_micro_Total_quadruple/rec_'+file_name[i].split('/')[3][6::] + '.csv', abs(tilde_signal_h1), delimiter=',')
    np.savetxt('./Bilby_rec_micro_Total_quadruple/freq_'+file_name[i].split('/')[3][6::] + '.csv', freq_h1, delimiter=',')

np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/match_filter_SNR.csv', match_filter_SNR, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_quadruple/max_match_det_lensed.csv', max_match_det_set, delimiter=',', fmt='%s')
