...
Total_inject_time = np.loadtxt('../Paper4_CE_Modify/Lensed_SampleResult_Micro_precessing/Total_inject_time.csv', delimiter=',')
...
with open('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/with_micro.txt', 'w') as f0:
    for i_ in range(0,256):
        i = int(AcceptLensIndex[i_]) #在SamplePama中的位置。
        imag_num = int(imagenum[i_])
        save_index = 0
        IndexSumimage = int(np.sum(imagenum[0:i_]))
        ...
        for j_ in range(imag_num):
            if SNR_network[IndexSumimage] > 12:
                if SNR_network[IndexSumimage] in SNR_selected_tmp:
                    ...
                    Ff_inter = np.loadtxt("../Paper4_MicroLens_Modify/Ffabs" + Lens_Type + "_precessing/Total" + str(i) + "_" + str(save_index) +  ".csv", delimiter=',')
                    ThetaF_inter = np.loadtxt("../Paper4_MicroLens_Modify/ThetaF" + Lens_Type + "_precessing/Total" + str(i) + "_" + str(save_index) +  ".csv", delimiter=',')
                    
                    Ff_inter = Ff_inter / sqrt_mag_abs
                    
                    
                    #input
                    with open('./input_micro_Total_precessing/H1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.in', 'w') as f:
                        f.write('/disk1/home/shanxk/work/Paper5_cWB_Modify/FRAMES_micro_Total_precessing/' + Lens_Type + '/H1/H-H1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096'+ '_' + str(i) + '_' + str(save_index) + '.gwf')
                    with open('./input_micro_Total_precessing/L1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.in', 'w') as f:
                        f.write('/disk1/home/shanxk/work/Paper5_cWB_Modify/FRAMES_micro_Total_precessing/' + Lens_Type + '/L1/L-L1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096'+ '_' + str(i) + '_' + str(save_index) + '.gwf')
                    with open('./input_micro_Total_precessing/V1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.in', 'w') as f:
                        f.write('/disk1/home/shanxk/work/Paper5_cWB_Modify/FRAMES_micro_Total_precessing/' + Lens_Type + '/V1/V-V1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096'+ '_' + str(i) + '_' + str(save_index) + '.gwf')
                    
                    # #copy file to FRAMES
                    shutil.copy('../Paper4_CE_Modify/Total_Sim_GW_Data_Lensed_' + Lens_Type + '_with_micro' + '_precessing/H1/H-H1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf', './FRAMES_micro_Total_precessing/' + Lens_Type + '/H1/H-H1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf')
                    shutil.copy('../Paper4_CE_Modify/Total_Sim_GW_Data_Lensed_' + Lens_Type + '_with_micro' + '_precessing/L1/L-L1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf', './FRAMES_micro_Total_precessing/' + Lens_Type + '/L1/L-L1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf')
                    shutil.copy('../Paper4_CE_Modify/Total_Sim_GW_Data_Lensed_' + Lens_Type + '_with_micro' + '_precessing/V1/V-V1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf', './FRAMES_micro_Total_precessing/' + Lens_Type + '/V1/V-V1_GWOSC_4KHZ_R1-' + str(int(Total_inject_time[index_in_total_inject_time])) + '-4096_' + str(i) + '_' + str(save_index) + '.gwf')
                    
                    #DQ
                    start_gps = int(Total_inject_time[index_in_total_inject_time] + 1000)
                    end_gps = int(start_gps + 3000)
                    with open('./DQ_micro_Total_precessing/H1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.txt', 'w') as f:
                        f.write("%s %s\n"%(start_gps, end_gps))
                    with open('./DQ_micro_Total_precessing/L1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.txt', 'w') as f:
                        f.write("%s %s\n"%(start_gps, end_gps))
                    with open('./DQ_micro_Total_precessing/V1_' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.txt', 'w') as f:
                        f.write("%s %s\n"%(start_gps, end_gps))
                    
                    excel_name_match.append(str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index))
                    inject_time_set.append(int(Total_inject_time[index_in_total_inject_time]))
                    
                    #顺便计算一下带微透镜波形和不带微透镜波形的match程度
                    luminosity_dis = Planck15.luminosity_distance(SampleParam[0][i]).value
                            # NOTE: Inclination runs from 0 to pi, with poles at 0 and pi
                            #       coa_phase runs from 0 to 2 pi. 
                    spin1z_tmp = spin1z[index_in_total_inject_time]
                    spin2z_tmp = spin2z[index_in_total_inject_time]
                    spin1x_tmp = spin1x[index_in_total_inject_time]
                    spin2x_tmp = spin2x[index_in_total_inject_time]
                    spin1y_tmp = spin1y[index_in_total_inject_time]
                    spin2y_tmp = spin2y[index_in_total_inject_time]  
                    ...
                    np.savetxt('./Micro_signal_Total_precessing/freq_'+str(int(i))+'_'+str(int(save_index))+'.csv', freq_h1, delimiter=',') 
                    np.savetxt('./Micro_signal_Total_precessing/lens_signal_'+str(int(i))+'_'+str(int(save_index))+'.csv', abs(tilde_signal_h1_lensed * sqrt_mag_abs), delimiter=',') 
                    np.savetxt('./Micro_signal_Total_precessing/time_'+str(int(i))+'_'+str(int(save_index))+'.csv', signal_h1.sample_times, delimiter=',') 
                    np.savetxt('./Micro_signal_Total_precessing/time_lens_signal_'+str(int(i))+'_'+str(int(save_index))+'.csv', signal_h1_lensed, delimiter=',') 
                    ...
                    plt.savefig('./IntermediaPlot/Micro_selected_precessing/' + str(int(Total_inject_time[index_in_total_inject_time])) + '_' + str(i) + '_' + str(save_index) + '.png', dpi=450)
                    plt.close()
                    ...
                    f0.write('../Paper4_CE_Modify/outdir_Lensed_'+Lens_Type+'_with_micro_precessing/outdir'+str(int(i))+'_'+str(int(save_index))+'/H1_L1_V1_result.json'+'\n')
            ...
            IndexSumimage += 1
# plt.plot(match)
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/match.csv', match, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/excel_name_match.csv', excel_name_match, delimiter=',', fmt='%s')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/SNR_selected.csv', SNR_selected, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/IndexInAccept.csv', IndexInAccept, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/inject_time_set.csv', inject_time_set, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/chirp_mass.csv', Mz, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/mass_ratio.csv', mass_ratio, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/merger_freq.csv', merger_freq_set, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/ringdown_freq.csv', ringdown_freq_set, delimiter=',')
np.savetxt('./Total_Data_start_time_and_snr_and_match_and_outdir_name_precessing/reference_det.csv', reference_det, delimiter=',', fmt='%s')