...
Bayes_LU = np.loadtxt('./Bayes_Unlens/overlap_LU.csv', delimiter=',')

Bayes_Unlens = np.append(Bayes_Unlens, Bayes_LU)
...
for i in range(len(Selected_index)):
    ...
    for j in range(len(Bayes_factor)):
        ...
        if FAP_per_pair_tmp == 0:
            ...
            FAP_per_pair.append(FAP_per_pair_tmp / 3)
        np.savetxt('./Bayes_Lens/FAP_per_pair_' + Selected_index[i].split('\n')[0] + '.csv', FAP_per_pair, delimiter=',')
...
for i in range(len(Selected_index)):
    try:
        FAP_per_pair = [float(np.loadtxt('./Bayes_Lens/FAP_per_pair_' + Selected_index[i].split('\n')[0] + '.csv', delimiter=','))]
    except TypeError:
        FAP_per_pair = np.loadtxt('./Bayes_Lens/FAP_per_pair_' + Selected_index[i].split('\n')[0] + '.csv', delimiter=',')
    for j in range(len(FAP_per_pair)):
        ...
        if FAP_per_pair[j] * 100000 < 1:
            ax2.scatter(FAP_per_pair[j] * 100000, i, color='red', marker='*', s=100)
        else:
            ax2.scatter(FAP_per_pair[j] * 100000, i, color='grey')
            
ax1.semilogx()
ax2.semilogx()
ax1.set_xlim(10**(-11), 10)
ax2.set_xlim(10**(-11) * 100000, 10 * 100000)
...
ax2.set_xticks([10**(0), 10**(1), 10**(2)], ['$1$', '$10$', '$100$'])
