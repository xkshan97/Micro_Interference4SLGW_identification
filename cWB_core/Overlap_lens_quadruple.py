...
for i in range(len(Selected_index)):
    ...
    z_s_set.append(SampleParam[0][int(tmp_index)])
np.savetxt('./PairResult_quadruple/z_s_set.csv', z_s_set, delimiter=',')
...
#with micro
filesminimum = os.listdir('../Paper4_CE_Modify/outdir_Lensed_Minimum_with_micro_quadruple')
filesminimum_index = [] #储存不带save_index的index
filessaddle = os.listdir('../Paper4_CE_Modify/outdir_Lensed_Saddle_with_micro_quadruple') 
filessaddle_index = []
#获得参数估计文件中的文件index
...
for i in range(len(Selected_index)):
    ...
    if len(index1 != 0): 
        for j in range(len(index1)):
            file_tmp1 = '../Paper4_CE_Modify/outdir_Lensed_Minimum_with_micro_quadruple/' + filesminimum[index1[j]] + '/H1_L1_V1_result.json'
    ...
    if len(index2 != 0):
        for j in range(len(index2)):
            file_tmp2 = '../Paper4_CE_Modify/outdir_Lensed_Saddle_with_micro_quadruple/' + filessaddle[index2[j]] + '/H1_L1_V1_result.json'
...
anker = [[0,1], [0,2], [0,3], [1,2], [1,3], [2,3]]
max_sky_localization = []
...
for i in tqdm(range(len(Selected_file_name_pair))):
    ...
    max_sky_localization_tmp = 100000
    for j in range(len(Selected_file_name_pair[i])):
        ...
        delta_ra = np.percentile(Poster[j][0], [99.85]) - np.percentile(Poster[j][0], [0.15])
        delta_dec = np.percentile(Poster[j][1], [99.85]) - np.percentile(Poster[j][1], [0.15])
        if np.pi * np.abs(delta_ra) / 2 * np.abs(delta_dec) / 2 < max_sky_localization_tmp:
            max_sky_localization_tmp = np.pi * np.abs(delta_ra) / 2 * np.abs(delta_dec) / 2
    ...
    max_sky_localization.append(max_sky_localization_tmp * 180 ** 2 / (np.pi ** 2))
    ...
    # plt.savefig('./IntermediaPlot_corner_Pair_quadruple/' + Selected_index[i].split('\n')[0] + '.png', dpi=450)
    # plt.close()
    
    #以第一个像为基准，计算Overlap，因为第一个像是被认证出的。
    #NA revision 2的版本中
    Bayes_factor = []
    for j in range(len(anker)):#range(len(Selected_file_name_pair[i]) -1):
        minset, maxset = [], []
        anker_tmp_1 = anker[j][0]
        anker_tmp_2 = anker[j][1]
        for ii in range(len(Poster[0])):
            minset.append(np.max([Poster[anker_tmp_1][ii].min(), Poster[anker_tmp_2][ii].min()]))
            maxset.append(np.min([Poster[anker_tmp_1][ii].max(), Poster[anker_tmp_2][ii].max()]))
        ra_, dec_ = np.mgrid[minset[0]:maxset[0]:100j, minset[1]:maxset[1]:100j]

        positions = np.vstack([ra_.ravel(), dec_.ravel()])

        
        kernel1 = sp.stats.gaussian_kde(Poster[anker_tmp_1])
        kernel2 = sp.stats.gaussian_kde(Poster[anker_tmp_2])
    ...
    np.savetxt('./Bayes_Lens_quadruple/' + Selected_index[i].split('\n')[0] + '.csv', Bayes_factor, delimiter=',')

np.savetxt('./PairResult_quadruple/max_sky_localization.csv', max_sky_localization, delimiter=',')
