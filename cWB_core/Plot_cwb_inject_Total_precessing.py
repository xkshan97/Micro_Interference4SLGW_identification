...
for i in tqdm(range(len(inject_time_set))):
    ...
    try:
        "读取cWB重构结果"
        cWB_path = './cWB_micro_precessing/data/' + str(int(inject_time_set[i])) + '_' + file_name[i].split('/')[3][6::]
    ...
    inject_freq = np.loadtxt('./Micro_signal_Total_precessing/freq_' + file_name[i].split('/')[3][6::] + '.csv', delimiter=',')
    inject_signal = np.loadtxt('./Micro_signal_Total_precessing/lens_signal_' + file_name[i].split('/')[3][6::] + '.csv', delimiter=',')
    inject_time = np.loadtxt('./Micro_signal_Total_precessing/time_' + file_name[i].split('/')[3][6::] + '.csv', delimiter=',')
    time_inject_signal = np.loadtxt('./Micro_signal_Total_precessing/time_lens_signal_' + file_name[i].split('/')[3][6::] + '.csv', delimiter=',')
    ...
    plt.legend()
    plt.grid()
    plt.savefig('IntermediaPlot/Micro_cWB_inject_precessing/freq_' + file_name[i].split('/')[3][6::] + '.png', dpi=450)
    plt.close()
    ...
    plt.legend()
    plt.grid()
    plt.savefig('IntermediaPlot/Micro_cWB_inject_precessing/time_' + file_name[i].split('/')[3][6::] + '.png', dpi=450)
    plt.close()
