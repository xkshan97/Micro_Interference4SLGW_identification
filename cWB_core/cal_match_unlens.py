...
inject_time_set = np.loadtxt('./Data_start_time_and_snr_and_match_and_outdir_name/Data_start_time_set_unlens.csv', delimiter=',')
with open('./Data_start_time_and_snr_and_match_and_outdir_name/reference_det.csv', 'r') as f1:
    reference_det = f1.readlines()
with open('./Data_start_time_and_snr_and_match_and_outdir_name/unlens.txt', 'r') as f:
    file_name = f.readlines()
#读取out_dir中的数据
freq_max = np.loadtxt('./Data_start_time_and_snr_and_match_and_outdir_name/ringdown_freq_unlens.csv', delimiter=',', encoding='UTF-8-sig') + 100
...
for i in tqdm(range(len(inject_time_set))):
    ...
    try:
        "读取cWB重构结果"
        cWB_path = './cWB_unlens/data/' + str(int(inject_time_set[i])) + '_' + file_name[i].split('/')[3][6::]
    ...
    for reference_det_tmp in ["H1", "L1", "V1"]:
        ...
        # j_max = np.where(data_unlens['posterior']['content']['log_likelihood'] == np.max(data_unlens['posterior']['content']['log_likelihood']))[0][0]
        # j_set = np.append(np.arange(start_index, end_index), j_max)
        # for j in j_set:
        j = np.where(data_unlens['posterior']['content']['log_likelihood'] == np.max(data_unlens['posterior']['content']['log_likelihood']))[0][0]
            
        q = data_unlens['posterior']['content']['mass_ratio'][j]
        Mz_o = data_unlens['posterior']['content']['chirp_mass'][j]
        luminosity_dis = data_unlens['posterior']['content']['luminosity_distance'][j]
        dec = data_unlens['posterior']['content']['dec'][j]
        ra = data_unlens['posterior']['content']['ra'][j]
        theta_jn = data_unlens['posterior']['content']['theta_jn'][j]
        psi = data_unlens['posterior']['content']['psi'][j]
        phase = data_unlens['posterior']['content']['phase'][j]
        a_1 = data_unlens['posterior']['content']['a_1'][j]
        a_2 = data_unlens['posterior']['content']['a_2'][j]
        ...
        tilt_1 = data_unlens['posterior']['content']['tilt_1'][j]
        tilt_2 = data_unlens['posterior']['content']['tilt_2'][j]
        phi_12 = data_unlens['posterior']['content']['phi_12'][j]
        phi_jl = data_unlens['posterior']['content']['phi_jl'][j]
        geocent_time = data_unlens['posterior']['content']['geocent_time'][j]
        m1_o = Mz_o * (1 + q)**(1/5) / q ** (3/5)
        m2_o = q * m1_o
        SNR_tmp = np.sqrt(data_unlens['posterior']['content']['H1_matched_filter_snr'][j]['real']**2 + data_unlens['posterior']['content']['L1_matched_filter_snr'][j]['real']**2 + data_unlens['posterior']['content']['V1_matched_filter_snr'][j]['real']**2)
    ...
    plt.grid()
    plt.savefig("./IntermediaPlot_match_unlens_complex/abs_" + file_name[i].split('/')[3][6::] + "_" + str(inject_time_set[i]) + ".png", dpi=450)
    plt.close()
    plt.figure(i)
    plt.title('reference_det = ' + max_match_det + ' SNR = ' + str(SNR_tmp) + 'match = ' + str(round(max_match,3)))
    ...
    # plt.ylim(10**(-30),10**(-22))
    plt.grid()
    plt.savefig("./IntermediaPlot_match_unlens_complex/phase_" + file_name[i].split('/')[3][6::] + "_" + str(inject_time_set[i]) + ".png", dpi=450)
    plt.close()
...
np.savetxt('./Data_start_time_and_snr_and_match_and_outdir_name_complex/match_filter_SNR_unlens.csv', match_filter_SNR, delimiter=',')
np.savetxt('./Data_start_time_and_snr_and_match_and_outdir_name_complex/max_match_det_unlens.csv', max_match_det_set, delimiter=',', fmt='%s')
