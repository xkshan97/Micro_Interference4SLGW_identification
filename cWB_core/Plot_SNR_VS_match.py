...
for i_ in tqdm(range(len(AcceptLensIndex[0:256]))):
...
# plt.plot(match)
np.savetxt('./Theory_match_total/match.csv', match, delimiter=',')
np.savetxt('./Theory_match_total/chirp_mass.csv', Mz, delimiter=',')
np.savetxt('./Theory_match_total/SNR_total.csv', SNR_gtr12_total, delimiter=',')
...
#图
Mz_cut = 20
match = np.loadtxt('./Theory_match_total/match.csv', delimiter=',')
Mz = np.loadtxt('./Theory_match_total/chirp_mass.csv', delimiter=',')
...
plt.savefig('./Plot_result/corner_SNR_VS_match.png', dpi=450)
